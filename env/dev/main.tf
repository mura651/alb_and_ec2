...
module "public_subnet_alb" {
  ...
  cidr_block_a   = var.public_subnet_1a_cidr
  cidr_block_c   = var.public_subnet_1c_cidr
}

module "nat_gateway" {
  source           = "../../modules/nat_gateway"
  vpc_id           = module.vpc.vpc_id
  public_subnet_id = module.public_subnet_alb.subnet_ids["${local.name_prefix}-subnet-public-1a"]
  name_prefix      = local.name_prefix
  az               = "ap-northeast-1a"
}
...
module "web" {
  ...
  subnet_id          = module.private_subnet_web_1a.subnet_ids["${local.name_prefix}-subnet-private-1a"]
...
module "alb" {
  ...
  subnet_ids      = values(module.public_subnet_alb.subnet_ids)
